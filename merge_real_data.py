...
missing_df = pd.read_csv('data/metadata/metadata_missing_types.csv')

print(f"\n主数据文件: {len(main_df)} 条")
print(f"缺失类型数据: {len(missing_df)} 条")

existing_ids = set(main_df['doc_id'].tolist())
new_records = missing_df[~missing_df['doc_id'].isin(existing_ids)]
